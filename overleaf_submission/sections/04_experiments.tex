...
To benchmark $M_{12}$ against contemporary paradigms, we evaluated standard feedforward segmentation architectures (U-Net and DeepLabV3+ with ResNet-50 backbones) on the exact same 40-image Kvasir-Sessile validation set. While our primary aim is mechanistic, Table~\ref{tab:sota_compare} demonstrates that curing the Feedback Trap elevates the recurrent FANet architecture to highly competitive SOTA performance, outperforming robust standard baselines on this extremely difficult subset.

\begin{table}[h]
\centering
\caption{Competitive Analysis on Kvasir-Sessile (Validation). Standard baselines evaluated zero-shot (ImageNet weights) vs. our trained recurrent models.}
