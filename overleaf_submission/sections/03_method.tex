...
\end{align}
Because $\text{detach}(\cdot)$ behaves as an identity in forward evaluation and zero in backward differentiation, the effective backward derivative is $1 - m_{\text{fg}}$. Since $m_{\text{fg}} \in [0, 1]$, this quantity is strictly non-negative and bounded in $[0, 1]$.
\end{proof}
...
\end{theorem}
\begin{proof}
Direct consequence of applying the stop-gradient operator $\text{detach}(\cdot)$ on $m_{\text{fg}}$.
\end{proof}
